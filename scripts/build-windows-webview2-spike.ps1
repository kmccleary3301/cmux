param(
    [string]$OutputPath = $(Join-Path $env:TEMP ("cmux_windows_webview2_spike_" + [guid]::NewGuid().Guid + '.exe')),
...
$helperSource = Join-Path $PSScriptRoot 'windows-webview2-browser-spike.cpp'
...
foreach ($requiredPath in @($helperSource, $sdkInclude, $sdkLib, $vcvars)) {
    if (-not (Test-Path $requiredPath)) {
        throw "Required WebView2 spike dependency not found: $requiredPath"
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Failed to build WebView2 browser spike helper"
}

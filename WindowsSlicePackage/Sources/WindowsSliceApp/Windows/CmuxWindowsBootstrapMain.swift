...
struct CmuxWindowsBootstrapMain {
    static func main() {
        ...
        if let restoredState = cmuxWindowsRestoreState(from: environment) {
    ...
    private static func cmuxWindowsRestoreState(from environment: [String: String]) -> CmuxCoreAppState? {
        guard let rawPath = environment["CMUX_WINDOWS_RESTORE_SESSION_SNAPSHOT_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !rawPath.isEmpty else {
            return nil
        }

        let snapshotURL = URL(fileURLWithPath: rawPath)
        guard let data = try? Data(contentsOf: snapshotURL),
              let snapshot = try? JSONDecoder().decode(SessionWorkspaceSnapshot.self, from: data) else {
            return nil
        }

        let workspaceID = UUID(uuidString: environment["CMUX_WINDOWS_RESTORE_WORKSPACE_ID"] ?? "") ?? UUID()
        let workspace = CmuxCoreWorkspace(id: workspaceID, snapshot: snapshot)
...
private enum CmuxWindowsBootstrapCommand: Sendable, Equatable {
    ...
    private static func resolve(
        ...
        case "open-shell-host", "launch-shell-host":
            return .openShellHost
...
enum CmuxWindowsBootstrapRuntime {
    static func launch(initialState: inout CmuxCoreAppState) {
        ...
        var shellHostSessions: [CmuxWindowsShellHostSession] = []
        ...
        persistSelectedWorkspaceSnapshot(from: initialState, environment: environment)
        ...
        for (index, bootstrapCommand) in bootstrapCommands.enumerated() {
            ...
            case .openShellHost:
                let shellHostOpened = CmuxWindowsShellHost.openSelectedWorkspace(state: initialState)
                if let session = shellHostOpened.session {
                    shellHostSessions.append(session)
                }
                shellHostReports.append(shellHostOpened.report)
            case .openBrowserPortal(let urlString, let title):
        ...
        let shellHostPreviewLines = shellHostReports.isEmpty
            ? nil
            : CmuxWindowsShellHost.previewLines(for: shellHostReports)
        let selectedWorkspace = initialState.selectedWorkspace
        let sessionSnapshotOutputPath = environment["CMUX_WINDOWS_SESSION_SNAPSHOT_OUTPUT_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if smokeOutputPath != nil {
            ...
            let report = CmuxWindowsSmokeReport(
                ...
                browserHostReports: browserReports.isEmpty ? nil : browserReports,
                shellHostPreviewLines: shellHostPreviewLines,
                notificationPreviewLines: notificationPreviewLines,
                ghosttyBridgePreviewLines: ghosttyBridgePreviewLines,
                ghosttyBridgeReports: bridgeReports.isEmpty ? nil : bridgeReports,
                shellHostReports: shellHostReports.isEmpty ? nil : shellHostReports,
                ...
                sessionSnapshotPath: sessionSnapshotOutputPath
            )
    ...
    private static func persistSelectedWorkspaceSnapshot(
        from state: CmuxCoreAppState,
        environment: [String: String]
    ) {
        guard let sessionSnapshotOutputPath = environment["CMUX_WINDOWS_SESSION_SNAPSHOT_OUTPUT_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !sessionSnapshotOutputPath.isEmpty,
              let selectedWorkspace = state.selectedWorkspace else {
            return
        }

        let snapshotURL = URL(fileURLWithPath: sessionSnapshotOutputPath)
        try? FileManager.default.createDirectory(
            at: snapshotURL.deletingLastPathComponent(),
            withIntermediateDirectories: true,
            attributes: nil
        )
        if let snapshotData = try? JSONEncoder().encode(selectedWorkspace.sessionSnapshot) {
